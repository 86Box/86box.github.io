...
The **Ensoniq AudioPCI** emulation was also expanded with Sound Blaster PCI cards based on the **ES1373** and **CT5880** chips. These were far more common than the previous ES1371 chip, which is better known through VMware's emulation than real cards. The newer chips are functionally equivalent and allow you to use later Creative drivers for Windows without running into [issues stemming from an incompatibility with the ES1371](https://richardg867.wordpress.com/2023/09/19/ensoniq-es1371-driver-bug/ "Ensoniq AudioPCI ES1371 and the driver bug nobody found"); though be careful with older ES1371-specific drivers, which may recognize the ES1373 but end up in crashes and other misbehavior.

**Small but important note:** similarly to the [AWE64 in v4.1.1](/2024/03/20/86box-v4-1-1.html#rom-set-update), users of the **Sound Blaster 16 PnP** and **Crystal CS4236B** should update the ROM set, as accurate Plug and Play ROMs were located. If this update is not present, these cards will **disappear from emulated machines** and the sound card selector until the ROM set is updated.
