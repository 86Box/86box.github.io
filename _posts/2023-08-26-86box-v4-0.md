...
The [**Mach8 and Mach32**](//en.wikipedia.org/wiki/ATI_Mach_series) were clones of the **IBM 8514/A** video accelerator which helped ATI gain market momentum in the early 1990s. 86Box now provides the first ever **functional emulation** of these cards. The Mach32 in particular was a popular card, available in [five different bus variants](//www.os2museum.com/wp/children-of-the-bus-wars/ "OS/2 Museum: Children of the Bus Wars") (of which we currently emulate ISA, MCA, VLB and PCI) as well as integrated to many motherboards, and had wide operating system support.

### Network

[**VDE**](//github.com/virtualsquare/vde-2) virtual networking is now supported for connecting 86Box machines and even other emulators to each other on Linux and macOS hosts. While Windows is notably not supported by VDE, we're working on a solution, so stay tuned. The [86Box documentation](//86box.readthedocs.io/en/v4.0/hardware/network.html#vde) has a full setup guide for creating a VDE network.
...
* Added translations: **Catalan** by [OBattler](//github.com/OBattler), <span class="emoji">🇸🇰</span> **Slovak** by [OBattler](//github.com/OBattler) and [**daviunic**](//github.com/daviunic)
...
* **Network**
  * Added [**VDE**](//github.com/virtualsquare/vde-2) virtual networking support for Linux and macOS hosts
...
* The **Ensoniq AudioPCI** no longer works with the 2001 Creative WDM driver (`SBPCI_WebDrvsV5_12_01.exe`); [this driver is designed for ES1373/CT5880 cards only, has identical BSOD issues on a real ES1371, and only works in VMware and previous versions of 86Box due to emulation inaccuracies](//messaroundery.net/2023/09/19/ensoniq-es1371-driver-bug "Ensoniq AudioPCI ES1371 and the driver bug nobody found")
