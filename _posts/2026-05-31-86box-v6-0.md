...
We now provide builds for Windows host systems with **Snapdragon**, **RTX Spark** and other **ARM processors** through the `86Box-NDR-Windows-ARM64` download. Intel and AMD systems should continue using the same `86Box-Windows-64` download as before. Note that ARM builds require Windows 11, and these currently lack some minor features relying on external components (Discord integration and the PostScript printer's PDF conversion).
...
* Moved **monitor sleep mode indicator** to the refresh rate indicator
...
* **Sound**
  ...
  * Added **Roland SC-55** MIDI synthesizer emulation ([setup required](//86box.readthedocs.io/en/v6.0/settings/sound.html#midi-out-device)\)
